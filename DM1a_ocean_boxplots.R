...
#---------------------------------------------------------
# Data parts removed till I work out how to do this for ocean data ------------------
#--------------------------------------------------------------
#### LOAD OBSERVATIONS AND ORGANISE DATA ####  
# files produced in Step0 extract site data
data_obs <- read.csv(file.path(dataobspath, "/ocean_data/margo_bench_ready.csv"), na.strings = "NA") %>% 
   dplyr::select (lat, lon, ocean_tas_anom, ocean_mtco_anom,	ocean_mtwa_anom, ref)
grid <- data_obs %>% dplyr::select (lat, lon)


# # select grid lat/lons for which we have Margo data  that geographical range (with averaged values for all variables)
...
sum_obs = summary(obs %>% filter (obs$ref == "Margo"))
obs2 = obs
# 
...
obs$ref <- factor(obs$ref , levels=c("Margo")) # reorder boxplots bottom to top
# 
...
# 
ggsave(bp,file=paste(plotpath,"DM_boxplots/boxplot_data_Margo.jpg", sep=""),width=12,height=7)
...
scales_y <- list(
  ...
  ocean_tas_anom = scale_y_continuous(breaks=scales::extended_breaks(n=4),limits=c(10,-20)),
...
ggsave(bp,file=paste(plotpath,"DM_boxplots/boxplot_ocean_data_model.jpg", sep=""),width=11,height=14)
#ggsave(bp,file=paste(plotpath,"DM_boxplots/boxplot_ocean_data_model.pdf", sep=""),width=11,height=14)
