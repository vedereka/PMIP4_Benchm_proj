...
for (obs_source in data_source_ls) {
  fname = (fname_ls[obs_source])
  #print(fname)
  ncin <- nc_open(fname)
  var_name <- (varname_ls[obs_source])
  ...
  obsraw <-
    lapply(fname, raster, obs_varname = SST) # for 2D netCDF files
...
#------------------------------------------------------------------------
  ...
  #for (source in data_source_ls) {
    ...
    for (region in region_ls$reg_name) {
      print(region) 
      out_filename <- paste (obs_source, "_obsNC_", region, ".csv", sep = "")
      ...
      for (mod_varname in mod_variable_ls) {
        ...
        #obs <- obsraw %>% filter (lat >= region_ls %>% filter (reg_name == region) %>% dplyr::select(min_lat) %>% as.numeric() &
        ...
        # 
        #obs <-
        #obsraw[, c(1, 2, which(colnames(obs) == obs_varname))] # select relevant variable
