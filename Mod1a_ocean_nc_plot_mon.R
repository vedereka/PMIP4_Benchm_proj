...
for (model in model_ls) {
  ...
  for (variab in var_ls) {
    ...
    for (per in period_sel) {
      ...
      # Land area fraction includes the ice areas, so can exclude this from the ocean squares
      ...
      if (max(sftgif,na.rm=T)==100){
        sftgif[sftgif > 50] <- NA
        sftgif[sftgif <= 50] <- 1 ## keep if < 50% ice
      } else {
        sftgif[sftgif > 0.5] <- NA # keep if less than 50% ice
        sftgif[sftgif <= 0.5] <- 1
      }
      ...
      #This should leave us only the ocean squares now
      ocean_mask <- sftgif * sftlf # use this to multiply it by the variable and remove land and ice gridcells
