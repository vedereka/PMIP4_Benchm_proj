...
for (region in region_ls$reg_name) {
  ...
  write.csv(sum_obs, paste(datapath,"/ocean_output/", region,"_summary_Margo.csv", sep=""))
  sum_obs = summary(obs_reg %>% filter (obs_reg$ref == "Tierney"))
  write.csv(sum_obs, paste(datapath,"/ocean_output/", region,"_summary_Tierney.csv", sep=""))
  sum_obs = summary(obs_reg %>% filter (obs_reg$ref == "AH"))
  write.csv(sum_obs, paste(datapath,"/ocean_output/", region,"_summary_AH.csv", sep=""))
  sum_obs = summary(obs_reg %>% filter (obs_reg$ref == "glomap"))
  write.csv(sum_obs, paste(datapath,"/ocean_output/", region,"_summary_glomap.csv", sep=""))
  sum_obs = summary(obs_reg %>% filter (obs_reg$ref == "kn"))
  write.csv(sum_obs, paste(datapath,"/ocean_output/", region,"_summary_kn.csv", sep=""))
  sum_obs = summary(obs_reg %>% filter (obs_reg$ref == "T_Grid"))
  write.csv(sum_obs, paste(datapath,"/ocean_output/", region,"_summary_T_Grid.csv", sep=""))
  #  }
  ...
  obs_coord = unique(data_Tgrid[,1:2])
  ...
  bpMod <-ggplot(na.omit(data_all), aes(x=region, y=value, fill=var)) +
    ...
    scale_fill_manual(name = element_blank(),
                      ...
                      labels = c("Margo","Tierney","AH", "glomap", "kn","T_Grid",modNames_ls[3], modNames_ls[2], modNames_ls[1],
