...
# Ocean Regions as defined by me 
region_ls <- rbind(c("Arctic", 75,90,-180,180),
                   c("NH", 0,90,-180,180),
                   c("NorthAtlantic",30,50,-60,-10), 
                   c("MidAtlantic",10,30,-90,25),
                   c("SAtlantic",-50,10,-70,15),
                   c("WPacNextratropics", 30,60,-180,-120),
                   c("EPacNextratropics", 30,60,130,180),
                   c("WPacSextratropics", -60,-30,-180,-120),
                   c("EPacSextratropics", -60,-30,130,180), 
                   c("WPacTropics", -30,30,-180,-80),
                   c("IndianOcean", -30,0,40,100), 
                   c("EPacTropics", -30,30,130,180)) %>% 
  as.data.frame (.) %>%
  dplyr::rename (reg_name = V1, min_lat = V2, max_lat = V3, min_lon = V4, max_lon = V5)
#------------------------------------------------------------------
source_ls <- c("Margo", "Margo_min", "Margo_max", "Tierney", "Tierney_min", "Tierney_max", "T_Grid", "T_Grid_min", "T_Grid_max", "AH", "AH_min", "AH_max", "kn", "kn_min", "kn_max", "glomap", "glomap_min", "glomap_max" ) 
    ...
    df_tbl <- #load all csv files in directory and rbind them
      ...
      map_df( ~ read.csv(.)) %>% `colnames<-`(
        c(
          ...
          "Had-GL","Had-IC","iLOVE-GL","iLOVE-IC",
          "INM","IPSL","MIROC","MPI"
        )
      )
...
refs_ls <- c("Margo", "Tierney", "T_Grid", "AH", "glomap", "kn") # min/max already used. 
